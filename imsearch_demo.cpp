...
void ImSearch::ShowDemoWindow(bool* p_open)
{
	...
	{
		static const char* selectedString = imguiExtensions[0];
		...
		if (ImGui::BeginCombo("##Extensions", selectedString))
		{
			if (ImSearch::BeginSearch())
			{
				...
				for (const char* extension : imguiExtensions)
				{
	...
	if (ImGui::TreeNode("Table"))
	{
		if (ImSearch::BeginSearch(ImSearchFlags_NoTextHighlighting))
		{
			...
			ImSearch::BeginHighlightZone(ImSearch::GetUserQuery());
			...
			if (ImGui::BeginTable("My beautiful table!", 4, flags))
			{
				enum UserId { Person_FirstName, Person_LastName, Person_Country, Person_Hobby };

				ImGui::TableSetupColumn("First Name", ImGuiTableColumnFlags_DefaultSort, 0.0f, Person_FirstName);
				ImGui::TableSetupColumn("Last Name", ImGuiTableColumnFlags_DefaultSort, 0.0f, Person_LastName);
				ImGui::TableSetupColumn("Country", ImGuiTableColumnFlags_DefaultSort, 0.0f, Person_Country);
				ImGui::TableSetupColumn("Hobby", ImGuiTableColumnFlags_DefaultSort, 0.0f, Person_Hobby);
					...
					if (sort_specs->SpecsDirty)
					{
						std::sort(people.begin(), people.end(), 
							[&](Person& lhs, Person& rhs)
							{
								...
								{
								case Person_FirstName: return strcmp(lhs.mFirstName, rhs.mFirstName) > 0;
								case Person_LastName: return strcmp(lhs.mLastName, rhs.mLastName) > 0;
								case Person_Country: return strcmp(lhs.mCountry, rhs.mCountry) > 0;
								case Person_Hobby: return strcmp(lhs.mHobby, rhs.mHobby) > 0;
								default: return true;
								}
							});
				...
				for (const Person& person : people)
				{
					if (ImSearch::PushSearchable(
						...
						[&](const char*)
						{
							...
							ImGui::TextUnformatted(person.mFirstName);

							ImGui::TableNextColumn();
							ImGui::TextUnformatted(person.mLastName);

							ImGui::TableNextColumn();
							ImGui::TextUnformatted(person.mCountry);

							ImGui::TableNextColumn();
							ImGui::TextUnformatted(person.mHobby);

							return true;
						}))
					{
						ImSearch::AddSynonym(person.mFirstName);
						ImSearch::AddSynonym(person.mLastName);
						ImSearch::AddSynonym(person.mCountry);
						ImSearch::AddSynonym(person.mHobby);
						ImSearch::PopSearchable();
					}
				}
